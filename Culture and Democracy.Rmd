...
data <- data_long %>% filter(variable %in% c("V127", "V128","V129","V130")) %>% 
  mutate(value_descriptive = plyr::mapvalues(value,from=c(1,2,3,4,-5,-4,-3,-2,-1), to=c("Very good or fairly good","Very good or fairly good","Fairly bad or very bad","Fairly bad or very bad","-5: Missing; RU: Inappropriate response","-4: Not asked in survey","-3: Not applicable","-2: No answer","-1: Don't know")))
...
data <- data %>% mutate(liberal = V133 + V136 + V139, egalitarian = V134 + V131 + V137, antiliberal = V132 + V135 + V138) %>% group_by(country_year) %>% select(liberal,egalitarian,antiliberal,weight) 
...
data <- WV6_Data_v_2015_04_18 %>% select(country_year,V127:V140, starts_with("Y"),weight)

toNAs <- function(val) ifelse(val < 0, NA, val) 
data <- cbind(country_year = data$country_year,plyr::numcolwise(toNAs)(data))
...
data <- data %>% mutate(elections_index = 5- V228A + V228B + V228C + V228D + 5 - V228E + 5 - V228F + V228G + V228H + 5 - V228I)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE,fig.height=20}
library(dplyr)
library(ggplot2)
library(scales)

data <- WV6_Data_v_2015_04_18 %>% select(country_year,V228A:V228I, weight) 

toNAs <- function(val) ifelse(val < 0, NA, val) 
data <- cbind(country_year = data$country_year,plyr::numcolwise(toNAs)(data))

data <- data %>% mutate(election_fairness = cut(5-V228A + 5-V228F + V228H, 4, labels=c("1: Not fair at all","2: Not very fair","3: Moderately fair","4: Fair"), include.lowest=TRUE, ordered_result=TRUE), media_fairness = cut(V228C + 5- V228E, 4, labels=c("1: Not fair at all","2: Not very fair","3: Moderately fair","4: Fair"), include.lowest=TRUE, ordered_result=TRUE), choice_extent = cut(V228B + 5-V228I, 4, labels=c("1: Not fair at all","2: Not very fair","3: Moderately fair","4: Fair"), include.lowest=TRUE, ordered_result=TRUE), wealthy_interference = cut(V228D + V228G, 4, labels=c("1: Not fair at all","2: Not very fair","3: Moderately fair","4: Fair"), include.lowest=TRUE, ordered_result=TRUE)) %>% group_by(country_year) %>% count(country_year,election_fairness,media_fairness,choice_extent,wealthy_interference,wt = weight, total=n()) 

data <- data %>% mutate(percent = n/total)

library(reshape2)
data <- melt(data, id.vars=c("country_year","n","percent","total"))
data <- data %>% group_by(country_year,variable,value) %>% summarise(percent = sum(percent))
data$variable <- plyr::mapvalues(data$variable, from = unique(data$variable), to = c("Election day fairness", "Media fairness", "Extent of choice","Money in politics"))
data <- na.omit(data)
data <- data %>% group_by(country_year,variable) %>% mutate(polarization = sd(percent))

data$country_year <- reorder(data$country_year,data$polarization)
ggplot(data=data,aes(x=value))+geom_bar(aes(fill=variable, y= percent),stat="identity",position="dodge")+theme_bw()+facet_wrap(~country_year,ncol=4) + labs(y ="Percent of respondents",x="Perception of fairness of electoral process",fill="Components of electoral process") + theme(legend.position="top", axis.text.x=element_text(angle=90)) + guides(fill=guide_legend(direction="horizontal", title.position="top",ncol=2)) + scale_fill_brewer(palette="PuOr") + scale_y_continuous(label=percent) 
...
data <- left_join(data,fh)
data <- left_join(data,polity)
data <- data %>% select(country,year,country_year,variable,polarization,total_fh_reversed,polity2,fh_cumulative_reversed,polity_cumulative2) %>% distinct()

V141 <- WV6_Data_v_2015_04_18 %>% group_by(country_year) %>% summarise(avg = weighted.mean(V141[V141 > 0 ],weight[V141 > 0 ])) %>% na.omit()

data <- left_join(data,V141)

qplot(data = data %>% filter(variable == "Election day fairness"),x=total_fh_reversed,y=polarization) + geom_smooth() + theme_bw() + geom_text(aes(label=country_year), size=2) + labs(x="Total FH score (PR+CL), reversed, in year of survey",y="Degree of agreement that election day is fair")

qplot(data = data %>% filter(variable == "Election day fairness"),x=fh_cumulative_reversed,y=polarization) + geom_smooth() + theme_bw() + geom_text(aes(label=country_year), size=2) + labs(x="Cumulative FH score (PR+CL), reversed",y="Degree of agreement that election day is fair")

qplot(data = data %>% filter(variable == "Election day fairness"),x=polity_cumulative2,y=polarization) + geom_smooth() + theme_bw() + geom_text(aes(label=country_year), size=2) + labs(x="Cumulative Polity2 score",y="Degree of agreement that election day is fair")

qplot(data = data %>% filter(variable == "Election day fairness"),x=avg,y=polarization) + geom_smooth() + theme_bw() + geom_text(aes(label=country_year), size=2) + labs(x="Avg. answer to V141: \nHow democratic is this country being governed today? (1-10)",y="Degree of agreement that election day is fair")
...
qplot(data = data,x=avg,y=polarization) + geom_smooth() + theme_bw() + geom_text(aes(label=country_year), size=2) + labs(x="Avg. answer to V141: \nHow democratic is this country being governed today? (1-10)",y="Degree of agreement on fairness") + facet_wrap(~variable)
